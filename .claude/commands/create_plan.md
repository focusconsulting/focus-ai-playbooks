...
This project has these areas:
- `api/` - Backend application (Python, Streamlit UI, Slack bot)
- `docs/` - Developer documentation (markdown)
- `cross-cutting` - Work spanning multiple areas

Please provide:
1. Which area does this work affect? (api, docs, or cross-cutting)
...
Tip: You can specify the area directly: `/create_plan api #123` or `/create_plan api bd-a1b2`
```
...
1. **Write the plan** to `thoughts/shared/plans/<area>/YYYY-MM-DD-issue-XXX-description.md`
   - Area options: `api/`, `docs/`, `cross-cutting/`
...
**File**: `api/checkin_api/path/to/file.py` (note: use full path from repo root)
...
- [ ] Type checking passes: `cd api && pyright`
